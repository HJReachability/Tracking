...
The control of the 5D model consists of the linear and angular acceleration, $(a, \alpha)$, and the disturbances are $(\dstb_x, \dstb_y)$.
...
\begin{aligned}
\TEB_\pstate(\tau) = \{\pstate: \exists s, \valfunc(\rtrans(\tstate-\ptmat\pstate), \thor-\tau) \le \underline\valfunc\} \\
\quad \text{(Finite time horizon)}\\
\TEB_{\pstate, \infty} = \{\pstate: \exists s, \valfunc_\infty(\rtrans(\tstate-\ptmat\pstate)) \le \underline\valfunc_\infty\} \\
