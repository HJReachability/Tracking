...
This error function is shown in Fig. \ref{fig:vf_TEB:5D3D} \SHnote{no it's not.}, and is used when one cares about tracking error in position.
...
For example, the error function, which corresponds to $\valfunc(\rstate, 0)$ \SHnote{haven't define V yet.  Also shouldn't this be V(r,T)?} in Fig. \ref{fig:vf_TEB:8D4D}, is defined in both position and velocity space; Fig. \ref{fig:valfuncRRT} shows yet another error function defined using the one-norm of the displacement between the two systems. 
In our pursuit-evasion game formulation, the tracking system tries to minimize the error, while the planning system and any disturbances experienced by the tracking system try to do the opposite -- maximize. \SHnote{discuss ideas for figures}
