...
\begin{figure}
  ...
  \caption{Projected slice of the error function (blue), value function (magenta), and the minimum value $\underline\valfunc$ (black) that determines the TEB according to \eqref{eq:TEBp} (top left); a slice of the TEB (bottom left), and the TEB in the error states with the sliced value of $\theta_r$ shown as the magenta plane (right).}
...
\begin{figure}
  ...
  \caption{FTH value function (left) and TEBs (right) for the 8D quadrotor tracking 4D double integrator example in Section \ref{sec:resultsMPC}. Note that $\valfunc(\rstate,0) = \errfunc(\rstate)$, which occurs at $\tau = 15$.}  
