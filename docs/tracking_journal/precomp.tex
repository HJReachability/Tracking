...
\label{eq:rdyn}
\begin{aligned}
\rstate = \rtrans(\tstate - \ptmat\pstate), \qquad \dot\rstate = \rdyn(\rstate, \tctrl, \pctrl, \dstb)
\end{aligned}
\end{equation}

\noindent where $\ptmat$ matches the common states of $\tstate$ and $\pstate$ by augmenting the state space of the planning model (as shown in Section \ref{sec:results}), and $\rtrans$ is a nonlinear transform that maps .
