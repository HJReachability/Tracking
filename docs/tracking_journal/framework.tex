...
\begin{figure}[h!]
	...
	\caption{Online framework \SHnote{fix planning/tracking system notation, path/trajectory planner notation}}
	\label{fig:fw_online}
\end{figure}
Another input to the planning algorithm is the set of augmented constraints $\constrAug$.  These are acquired by updating constraints $\constr$ and accordingly updating the sensed constraints $\constrSense$ in the environment.  This can be done, for example, by sensing the environment for obstacles\footnote{\SHnote{conditions on sensing}}. Next, $\constrSense$ is augmented  by the precomputed TEB to produce the augmented constraints $\constrAug$. 
