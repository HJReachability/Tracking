...
\begin{algorithm}	
    ...
    \STATE (or $\constrAug \leftarrow \constrSense \ominus \TEB_{\estate, \infty}$) 
		...
		\STATE {use optimal tracking controller: $\tctrl \leftarrow \tctrl^*$ in \eqref{eq:opt_ctrl}}
...
The sensed constraints are augmented by $\TEB_\estate(\cdot)$ in a time-varying fashion (or by $\TEB_{\estate, \infty}$ in a time-invariant fashion) using the Minkowski difference, denoted ``$\ominus$''.\footnote{\SHnote{Sylvia to do: explain how we often don't take the minkowski sum and instead just expland obstacles by the largest distance in each dimensions}} 
