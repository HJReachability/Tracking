...
The planning is done using a relatively simplified model of the system, called the planning model. 
...
The tracking model is a more accurate representation of the autonomous system dynamics, and in general may be nonlinear and higher-dimensional than the planning model presented in Section \ref{sec:planning_model}. 
...
For navigation in unknown environments, fast replanning is necessary, so the planning model is typically one that allows the desired path or trajectory planning algorithm to operate in real time.
...
When the planning model of the system reaches sufficiently within the goal region, the autonomous system would be at the goal as well.
