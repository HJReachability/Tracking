...
\example{We introduce a running example for illustration throughout the paper. In this example a car will have to navigate through an environment with a priori unknown obstacles ($\tconstr^\complement$) towards a goal ($\tgoal$). The tracking model of the car is represented by the following five-dimensional dynamics:
\begin{equation}
\label{eq:5Ddyn}
\begin{bmatrix}
\dot x\\
...
\end{bmatrix},
\end{equation}
\noindent where $(\hat x, \hat y, \hat\theta)$ represent the pose (position and heading) of the 3D car model. Here the speed $\hat v$ is a constant, and the turn rate $\hat \omega$ is the control. The planning model must reach its goal $\goal$ while avoiding obstacles represented by $\constrAug(t)$.
}
