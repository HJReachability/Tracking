...
The tracking model is a representation of the true autonomous system dynamics, and in general may be nonlinear or high-dimensional. Let $\tstate$ represent the state variables of the tracking model. The evolution of the dynamics satisfy the ordinary differential equation: \MCnote{I think the subscript $s$ and $p$ are confusing because they are function arguments. Consider using different letters for dynamics? This could be part of Somil's confusion}
\begin{equation}
\begin{aligned}
\label{eq:tdyn}
...
The goals of the paper are threefold: \MCnote{Perhaps this sort of short summary would be appropriate in the introduction. In the goals section we can be a bit more detailed (maybe even taking up to half a column)}
\begin{enumerate}
	...
	\item To demonstrate the tool and framework by safely tracking a 3D RRT planner with a 10D near-hover quadrotor model in real time. \MCnote{I wouldn't talk specifically about RRT here.}
\end{enumerate}
