...
Relative state: \MCnote{$\tvmat$ is a matrix that subsamples the states. It could be a more general transformation which would be useful for doing rotations (eg. in air3D there's also rotation so that one vehicle is facing right)}
\begin{equation}
