...
\SHnote{1. change real and virtual system to tracking and planning system in both the text and the figures.\\ 2. Might not need to go into as much detail as we currently are here.\\ 3. Should we introduce the frameworks in the same order that we go through them in the paper (i.e. offline, online, hybrid controller)?\\
4. explain that having a tracking bound around the planner in the workspace is equivalent to expanding the obstacles by the tracking bound amount in the configuration space\\}
...
\label{fig:fw_online}
\SHnote{if there's room I think it would be nice to keep the labels on the arrows}
\end{figure}
