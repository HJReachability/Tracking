...
The framework of the FASTrack tool is summarized in Figs. \ref{fig:fw_online}, \ref{fig:hybrid_ctrl}, \ref{fig:fw_offline}. The online real-time framework is shown in Fig. \ref{fig:fw_online}. At the center of this framework is the path or trajectory planner, to which our framework is agnostic: any may be used (e.g. MPC, RRT, neural networks, etc.). We will present an example with an RRT planner in Section \ref{sec:results}.
...
\begin{figure}[h!]
\includegraphics[width=0.95\columnwidth]{fig/framework_online}
