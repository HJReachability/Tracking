...
\begin{algorithm}	
	...
	\begin{algorithmic}[1]
		...
		\IF{$\rstate_{next}$ is on boundary $\TEB_\pstate(\tstate)$} 
...
The control $\tctrl^*$ is then applied to the physical system in the tracking block (lines \ref{ln:trackingStart}-\ref{ln:trackingEnd}) for a time period of $\dt$. The next state is saved as $\tstate_{next}$. We repeat this process until the planning goal has been reached.