...
%         dist vmin (m/s)    <= [d(1); d(2); d(3)] <= dist vmax (m/s)
%         dist amin (m/s^2)  <= [d(4); d(5); d(6)] <= dist amax (m/s^2)
...
switch dim
  ...
  case 3
    dx = x{dims==6} - d{3};
  case 4
    dx = obj.grav * tan(u{1}) - d{4};
  case 5
    dx = - obj.grav * tan(u{2}) - d{5};
  case 6
    dx = u{3} - obj.grav - d{6};
    
  otherwise
    error('Only dimension 1-6 are defined for dynamics of Q6D!')
end
end
