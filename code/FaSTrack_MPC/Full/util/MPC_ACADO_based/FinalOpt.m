function [state, control, runtime, out] = FinalOpt(x0,xF,N,Ts,tt,TEB,tau,stateWS,controlWS,A,b,dmin)
% Refine iterative computation results
    ...
    BEGIN_ACADO;  
        ...
        %% set ACADO
        acadoSet('problemname', 'FinalOpt');            % Set your problemname. If you 
...
%         ocp.subjectTo( 'AT_END', xF(2)-0.4 <=  y  <= xF(2)+0.4  );
        ...
        % time varying TEB constraints on velocity
        TsWS = Ts*stateWS(1,6);
        TVTEB = getTVTEB(tt,TEB,tau,TsWS,N);
        for i = 2:N+1
%             ocp.subjectTo(  i-1, vx^2 + vy^2 <= (2.5-TVTEB(i,2))^2  );
            ocp.subjectTo(  i-1, norm([vx;vy]) <= 2.5-TVTEB(i,2)  );
        end
        
        load TEB_save
        load currentTime
        TEB_save{end+1} = TVTEB(:,2);
        currentTime(end+1) = tt;
        save TEB_save TEB_save
        save currentTime currentTime
        
%         load vCons
%         vCons = [vCons; tt 2.5-max(TVTEB(i,2))];
%         save vCons vCons
    ...
    tic()
    out = FinalOpt_RUN();                               % Run the test. The name of the RUN file
