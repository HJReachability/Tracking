...
if strcmp(uMode, 'max')
  ...
  if any(dims==2)
    uOpt{1} = ((-deriv{dims==2})>=0)*obj.uMin(1) + ((-deriv{dims==2})<0)*obj.uMax(1);
  end
  ...
  if any(dims == 4)
    ...
    %planner tries to min
  if any(dims == 6)
     uOpt{3} = ((-deriv{dims==6})>=0)*obj.uMin(3) + ((-deriv{dims==6})<0)*obj.uMax(3);
  end
  ...
  if any(dims == 8)
    ...
    %planner tries to min
  if any(dims==10)
     uOpt{5} = ((-deriv{dims==10})>=0)*obj.uMin(5) + ((-deriv{dims==10})<0)*obj.uMax(5);
  end
...
elseif strcmp(uMode, 'min')
  ...
  if any(dims==2)
    uOpt{1} = ((-deriv{dims==2})>=0)*obj.uMax(1) + ((-deriv{dims==2})<0)*obj.uMin(1);
  end
  ...
  %planner tries to max
  if any(dims==10)
     uOpt{5} = ((-deriv{dims==10})>=0)*obj.uMax(5) + ((-deriv{dims==10})<0)*obj.uMin(5);
  end
