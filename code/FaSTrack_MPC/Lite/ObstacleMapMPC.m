classdef ObstacleMapMPC < handle
  % Obstacle map class for FaSTrack+MPC
  ...
  properties
    ...
    padded_obs
    vOb
    nObs % number of individual obstacles
  ...
  methods
    ...
    %% SenseAndUpdate
    % sense obstacles that are within sense_range of x0
    % then update local and padded obstacles
    function sense_update(self, x0, sense_radius, track_err, iter)
        self.local_obs = updateLocal(x0, self.global_obs, self.local_obs, sense_radius);
        self.padded_obs = updatePadded(self.local_obs, track_err, iter);
    ...
    function plotPadded(self, iter)      
      ...
      if iter== 50 || iter== 80
        self.hP2 = plotPadded(self.padded_obs, color, linestyle, extraArgs);
      else
        delete(self.hP)
                ...
                if j == i
                    ...
                    for k = 1:4
                        if poly.contains(lOb{j,k}) && norm(lOb{j,k}) < 100                            
                            ...
                            flag = true;
%                             fprintf('\n******************************************\n');
%                             fprintf('***     Obstacle overlapping detected      ***\n');
%                             fprintf('******************************************\n');                        
                        end
                        if flag
                            break
